...
namespace Invector.ItemManager
{
    public class vWeaponHolderManager : MonoBehaviour
    {
            ...
            if (itemInArea != null && itemInArea.Count > 0 && holderAreas.ContainsKey(equipArea.equipPointName))
            {
                for (int i = 0; i < itemInArea.Count; i++)
                {
                    if (itemInArea[i].item != null)
                    {
	                    ...
	                    (equipArea.currentEquipedItem != null && equipArea.currentEquipedItem != itemInArea[i]));
	                    
                        if (holder)
                        {
	                        holder.SetActiveHolder(true);
                        	holder.SetActiveWeapon(!itemManager.inventory.isOpen);
                        }
                    }
        ...
        public void UnequipWeapon(vEquipArea equipArea, vItem item)
        {
            ...
            if ((itemManager.inventory != null && itemManager.inventory.isOpen) && holderAreas.ContainsKey(equipArea.equipPointName))
            {
