...
public class v_AIWeaponsControl : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        if (itemCollection && ai && manager)
        {
            ...
            if (useRightWeapon)
            {
                if (randomRightWeapon) GetRandomWeapon(ref rightWeaponItem, MeleeType.OnlyAttack);
                else GetItemWeapon(rightWeaponID, ref rightWeaponItem, MeleeType.OnlyAttack);
            }

            if (useLeftWeapon)
            {
                if (randomLeftWeapon) GetRandomWeapon(ref leftWeaponItem, MeleeType.OnlyDefense);
                else GetItemWeapon(leftWeaponID, ref leftWeaponItem, MeleeType.OnlyDefense);
            }
    ...
    void GetItemWeapon(int id, ref vItem weaponItem, MeleeType type)
    {
        if (weaponItems.Count > 0)
        {
            weaponItem = weaponItems.Find(_item => _item.id == id &&
                        ...
                        (_item.originalObject.GetComponent<vMeleeWeapon>().meleeType == MeleeType.AttackAndDefense || _item.originalObject.GetComponent<vMeleeWeapon>().meleeType == type));

            weaponItems.Remove(weaponItem);
        }
    }

    void GetRandomWeapon(ref vItem weaponItem, MeleeType type)
    {
        if (weaponItems.Count > 0)
        {
    ...
    IEnumerator UnequipItemRoutine(bool flipID, vItem item, GameObject weapon)
    {
        ...
        while (inUnequip || ai.actions || ai.inAttack || ai.ragdolled)
        {
            yield return new WaitForEndOfFrame();
        }
