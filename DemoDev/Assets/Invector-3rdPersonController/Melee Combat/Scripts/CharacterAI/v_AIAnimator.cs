    ...
    public class v_AIAnimator : v_AIMotor
    {
        ...
        void OnAnimatorMove()
        {
            ...
            if (OnStrafeArea && !actions && target != null && canSeeTarget && currentHealth > 0f)
            {
                ...
                float step = (meleeManager != null && inAttack) ? attackRotationSpeed * Time.deltaTime : (strafeRotationSpeed * Time.deltaTime);
        ...
        {
            onGround = agent.enabled ? agent.isOnNavMesh : rolling ? true : groundDistance <= groundCheckDistance;
        ...
        protected void RollAnimation()
        {
            ...
            rolling = baseLayerInfo.IsName("Roll");
            if (rolling)
            {
        ...
        {
            if (animator != null && animator.enabled && !rolling)
            {               
