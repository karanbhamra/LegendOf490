...
namespace Invector
{
    public class v_AIController : v_AIAnimator, vIMeleeFighter
    {
        ...
        {
            if (!inAttack && !actions && attackCount > 0 && !lockMovement)
            {
                sideMovement = GetRandonSide();
        ...
        {            
            ...
            if (canSeeTarget && !damage.ignoreDefense && isBlocking && meleeManager != null && meleeManager.CanBlockAttack(attacker.Character().transform.position))
            {
