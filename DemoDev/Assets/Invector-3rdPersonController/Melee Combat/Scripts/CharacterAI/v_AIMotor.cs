...
namespace Invector
{
    public class v_AIMotor : vCharacter
    {
        ...
        public bool actions { get { return lockMovement || rolling; } }
        ...
        protected void CheckGroundDistance()
        {
            if (_capsuleCollider != null)
            {
                ...
                if (!actions && !rolling && groundDistance < 0.3f)
                {
                    //if (!agent.enabled)
                    {                                     
                        if (currentHealth > 0)
        ...
        public void CheckTarget()
        {
            ...
            RaycastHit hit;
            if (canSeeTargetInteration ==0 && !Physics.Linecast(head.position, top, out hit, sphereSensor.obstacleLayer))
            {
                canSeeTarget = true;
                canSeeTargetInteration = 0;
                return;
            ...
            else if (canSeeTargetInteration == 1 && !Physics.Linecast(head.position, bottom, out hit, sphereSensor.obstacleLayer))
            {
                canSeeTarget = true;
                canSeeTargetInteration = 0;
                return;
            }
            else if (canSeeTargetInteration == 2 && !Physics.Linecast(head.position, colliderTarget.bounds.center, out hit, sphereSensor.obstacleLayer))
            {
                canSeeTarget = true;
                canSeeTargetInteration = 0;
                return;
                ...
                canSeeTargetInteration++;
            if (canSeeTargetInteration > 1)
                canSeeTargetInteration = 0;
            canSeeTarget = false;
        }               
        ...
        public override void TakeDamage(Damage damage,bool hitReaction)
        {
	        ...
	        if (rolling || currentHealth <= 0 || !animator.enabled) return;
            ...
            if (animator != null && animator.enabled && !damage.activeRagdoll && !rolling)
            {
        ...
        {
            if (inAttack || actions) return false;
        ...
        protected IEnumerator CheckChanceToBlock(float chance, float timeToEnter)
        {
            ...
            if (randomBlock < chance && randomBlock > 0 && !isBlocking)
            {
                ...
                isBlocking = target == null || (actions) || inAttack ? false : true;
